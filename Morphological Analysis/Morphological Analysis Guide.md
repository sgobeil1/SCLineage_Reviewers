...
Main demo-ready pipeline. It loads Cell_Info.csv, scans SWC files, performs angle analysis and final morphotype classification, and computes complexity directly from SWC using T100. It can also run in SWC-only mode if no CSV is provided.
Outputs (examples):
...
Computes per-cell and per-clone Sholl curves, total intersections, maximum intersections, and summary plots.
Outputs (examples):
...
Computes a clone heterogeneity index from the morphotype table generated by MorphotypeClassifier.py.
Outputs:
...
Runs permutation / bootstrap analysis of clone morphotype richness and heterogeneity, comparing real data to homogeneous and heterogeneous simulated datasets.

*HistPlotting.py (paper-figure helper)*
Additional plotting helper that expects inputs produced by the scripts above and/or manuscript tables. Not required for the demo.
